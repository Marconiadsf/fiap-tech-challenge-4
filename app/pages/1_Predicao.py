...
# Garante que mesmo que o usuário acesse esta página diretamente, o modelo estará
# disponível — desde que a sessão exista.
...
with st.form("prediction_form"):
    ...
    with c6:
        ncp = st.slider(
            ...
            4,
            3,
            help="1 = uma refeição · 2 = duas · 3 = três · 4 = quatro ou mais"
        )
    ...
    with c8:
        ch2o = st.slider(
            "Consumo de água por dia em litros (CH2O)",
            1,
            3,
            2,
    ...
    with c9:
        faf = st.slider(
            ...
            3,
            1,
            help="0 = nenhuma · 1 = 1–2x/sem · 2 = 3–4x/sem · 3 = 5x/sem ou mais"
        )
...
if submitted:
    ...
    with col_prob:
        prob_df = pd.DataFrame({
            "Classe_original": le.classes_,
            "Classe": [CLASS_LABELS.get(c, c) for c in le.classes_],
        ...
        fig = px.bar(
            ...
            color="Classe_original",
            color_discrete_map={
                c: CLASS_COLORS.get(c, "#6366f1")
    ...
    if caec in ["Frequently", "Always"]:
        risk_factors.append("🍪 **Consumo entre refeições:** consumo frequente de alimentos entre refeições")

    if mtrans == "Automobile":
        risk_factors.append("🚗 **Transporte:** uso habitual de automóvel, associado a menor gasto calórico")
